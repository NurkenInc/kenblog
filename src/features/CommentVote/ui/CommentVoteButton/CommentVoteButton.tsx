...
import { CommentVoteRequest } from '../../model/validator/votes';
...
export const CommentVoteButton = (props: CommentVoteButtonProps) => {
  const {
    commentId,
    initialDislikes,
    initialLikes,
  ...
  const { mutate: vote } = useMutation({
    mutationFn: async (voteType: VoteType) => {
      ...
      await axios.patch('/api/posts/comment/vote', payload);
    },
    onError: (err) => {
  ...
  return (
    <div className='flex gap-4 sm:gap-0 pr-6 sm:w-20 pb-4 sm:pb-0'>
      <Button onClick={onVote('UP')} size='sm' variant='ghost' aria-label='upvote'>
